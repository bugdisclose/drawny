...
export default function ShareButton({ viewport, onCaptureSnapshot, openRef }: ShareButtonProps) {
    ...
    // Subtle glow nudge — random interval
        ...
        const scheduleGlow = () => {
            ...
            return setTimeout(() => {
                if (!showModal) {
                    setGlowKey(k => k + 1);
                    setIsGlowing(true);
                    // Stop glow after animation completes (2s)
                    setTimeout(() => setIsGlowing(false), 2000);
                }
                timerId = scheduleGlow();
            }, delay);
        };
        let timerId = scheduleGlow();
    ...
    return (
        <>
            <div className={styles.container}>
                <button
                    key={glowKey}
                    className={`${styles.shareButton} ${isGlowing ? styles.shareGlow : ''}`}
