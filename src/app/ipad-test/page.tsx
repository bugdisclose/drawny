...
export default function IPadTestPage() {
  ...
  useEffect(() => {
    ...
    const onTouch = (e: TouchEvent) => addLog(`${e.type} touches=${e.touches.length} target=${(e.target as Element)?.className?.substring(0, 40)}`);
    const onPointer = (e: PointerEvent) => addLog(`${e.type} ptrType=${e.pointerType} target=${(e.target as Element)?.className?.substring(0, 40)}`);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <Excalidraw
          excalidrawAPI={() => { addLog('Excalidraw API ready'); setReady(true); }}
          initialData={{
            appState: {
          ...
          onChange={(elements) => {
            if (elements.length > 0) {
              addLog(`onChange: ${elements.length} elements, last type=${elements[elements.length - 1]?.type}`);
            }
          }}
          autoFocus={true}
        />
